...
SELECT first_name, last_name, content, t.name FROM quotes JOIN quote_topic qt ON quotes.id = qt.quote_id JOIN topics t ON qt.topic_id = t.id JOIN authors a ON quotes.author_id = a.id;